...
export async function verifyDeactivation(
  ...
  // 6. Post-chat: reactivation option
  console.log("");
  const postAction = await select({
    message: "Que souhaitez-vous faire ?",
    choices: [
      { name: "Reactiver le compte", value: "reactivate" },
  ...
  try {
    await taQ.reactivateAccount(conn, account.trading_account_uuid, "");
